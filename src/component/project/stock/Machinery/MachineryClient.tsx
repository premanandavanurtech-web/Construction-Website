    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
            ...
            {machinery.length === 0 ? (
              <tr>
                <td colSpan={8} className="py-12 text-center text-gray-400">
                  {items.length === 0
                    ? "No items loaded — check debug panel above"
              ...
              machinery.map((item, i) => (
                ...
                >
                  <td className="px-4  text-black py-3 font-medium">
                    {item.name}
                  </td>
                  ...
                  <td className="px-4  text-black py-3">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${item.current <= item.min ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                    >
                      {item.status}
                    </span>
                  </td>
      ...
      <ConfirmDeleteModal
        ...
        onConfirm={() => {
          ...
          localStorage.removeItem(`stock-${projectId}-${deleteItem.name}`);
          setDeleteItem(null);
          loadStock();
        }}
      />
