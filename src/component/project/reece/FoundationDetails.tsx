...
export default function FoundationDetails() {
  ...
  const update = (key: keyof FoundationData, value: any) =>
  ...
  const handleSubmit = () => {
    ...
    alert("Foundation details saved (valid for 1 week)");
  };

  /* -------- AUTO EXPIRE -------- */
  if (typeof window !== "undefined") {
    const stored = localStorage.getItem("foundationDetails");
    if (stored) {
      const parsed = JSON.parse(stored);
      if (Date.now() - parsed.savedAt > ONE_WEEK) {
        localStorage.removeItem("foundationDetails");
      }
    }
  }
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Deep, Raft, Strip..."
            onChange={(e) =>
              update("foundationType", e.target.value)
            }
          />
        </div>
        ...
        <div>
          ...
          <select
            className="w-full h-10 px-3 rounded-md bg-gray-100 outline-none"
            onChange={(e) =>
              update("excavationRequired", e.target.value)
            }
          >
