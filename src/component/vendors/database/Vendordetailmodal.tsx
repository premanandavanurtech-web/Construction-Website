...
import { X, UserCircle2, Globe, Phone, Mail, MapPin, Building2, CreditCard, Landmark, FileText, BadgeCheck, Hash, Download, ExternalLink } from "lucide-react";
    ...
    <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-2.5 py-2 hover:border-gray-300 transition">
      ...
      <div className="w-8 h-8 rounded-lg bg-gray-100 overflow-hidden shrink-0 flex items-center justify-center">
        {isImage ? (
          ...
          <FileText className={`w-4 h-4 ${isPdf ? "text-red-400" : "text-gray-400"}`} />
        )}
      </div>
      ...
      <div className="flex gap-1 shrink-0">
        <button onClick={handleOpen} title="View" className="w-6 h-6 rounded-md flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition">
          <ExternalLink className="w-3 h-3" />
        </button>
        <button onClick={handleDownload} title="Download" className="w-6 h-6 rounded-md flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition">
      ...
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b bg-white shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${
              ...
              : vendor.status === "In Progress" ? "bg-yellow-100 text-yellow-700"
              : "bg-gray-100 text-gray-500"
            }`}>
              {vendor.status}
            </span>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="relative bg-gradient-to-br from-gray-900 to-gray-700 px-6 pt-8 pb-16">
            ...
            <div className="relative flex items-end gap-5">
              <div className="shrink-0">
                {vendor.profilePhoto ? (
                  <img src={vendor.profilePhoto} alt={vendor.name} className="w-20 h-20 rounded-2xl object-cover border-2 border-white/30 shadow-xl" />
                ) : (
          ...
          <div className="px-6 pb-6 space-y-4">
            ...
            {/* Documents — ✅ 2-column grid so 2 per row */}
            <div className="rounded-xl border border-gray-100 bg-gray-50/60 p-4">
        ...
        <div className="px-6 py-3 border-t bg-white shrink-0 flex justify-end">
          <button onClick={onClose} className="px-5 py-2 text-sm font-medium rounded-lg bg-gray-800 text-white hover:bg-gray-700 transition">
            Close
          </button>
        </div>
      </div>
